...
int init_ore_blk_params(ore_blk_params params, uint32_t nbits,
                        uint32_t block_len) {
 ...
 *         (see errors.h for the full list of possible error codes)
 */
static inline int _eval_keyed_hash_aes_ro(uint8_t *out, const block key,
                                          const block val) {
  AES_KEY aes_key;
 ...
 *         (see errors.h for the full list of possible error codes)
 */
static inline int _eval_keyed_hash_sha256(uint8_t *out, const block key,
                                          const block val) {
 ...
 *         (see errors.h for the full list of possible error codes)
 */
static inline int _eval_keyed_hash(uint8_t *out, const block key,
                                   const block val) {
#ifdef USE_AES_RO
 ...
 * @param nblocks  The number of hash function evaluations
 * @param keys     The vector of keys (of length nblocks) used to apply the hash
 * function
 * @param val      The value to evaluate the hash functions on
 *
 * @return ERROR_NONE on success and a corresponding error code on failure
 ...
 *         (see errors.h for the full list of possible error codes)
 */
static int _ore_blk_encrypt_block(byte *comp_left, byte *comp_right,
                                  ore_blk_secret_key sk, block nonce,
                                  uint64_t block_ind, uint64_t prefix,
                                  uint64_t val) {
  ...
  block prp_key_buf;
  ERR_CHECK(
      aes_eval(&prp_key_buf, &sk->prp_key, MAKE_BLOCK(block_ind, prefix)));
  ...
  ERR_CHECK(aes_eval(&key, &sk->prf_key,
                     MAKE_BLOCK(block_ind, prefix_shifted | pix)));
  ...
  if (bytes_written < expected_len) {
    memmove(comp_right + (expected_len - bytes_written), comp_right,
            bytes_written);
...
int ore_blk_encrypt_ui(ore_blk_ciphertext ctxt, ore_blk_secret_key sk,
                       uint64_t msg) {
  if (!sk->initialized) {
  ...
  for (int i = 0; i < nblocks; i++) {
    ...
    ERR_CHECK(_ore_blk_encrypt_block(comp_left, comp_right, sk, nonce, i,
                                     prefix, cur_block));
...
int ore_blk_compare(int *result_p, ore_blk_ciphertext ctxt1,
  ...
  bool is_equal = true;
  for (int i = 0; i < nblocks; i++) {
    if (memcmp(ctxt1->comp_left + offset_left, ctxt2->comp_left + offset_left,
               AES_BLOCK_LEN) != 0) {
      is_equal = false;
      break;
    }
  ...
  uint64_t index = 0;
  memcpy(&index, ctxt1->comp_left + offset_left + AES_KEY_BYTES,
         CEIL(block_len, 8));

  mpz_t ctxt_block;
  mpz_init(ctxt_block);
  mpz_import(ctxt_block, CEIL(nslots, 8), 1, 1, -1, 0,
             ctxt2->comp_right + offset_right);
...
int ore_blk_ciphertext_size(ore_blk_params params) {
  return _ore_blk_ciphertext_len_left(params) +
         _ore_blk_ciphertext_len_right(params);
}
